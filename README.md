...
│\
├─ server/\
│ ├─ main.py # Flask-сервер для приёма логов\
│ ├─ config.py # Конфигурация сервера (JWT, MAC, IP, БД)\
│ ├─ auth_middleware.py # Middleware проверки JWT, MAC и IP\
│ └─ admin/\
│ ├─ cli.py # Админская CLI-консоль\
│ └─ admin_views.py # Вспомогательные функции для админа\
│\
├─ client/\
│ ├─ main.py # Клиент: читает и отправляет логи\
│ ├─ auth.py # Аутентификация и работа с JWT\
│ ├─ generator.py # Генератор логов (норма + атаки)\
│ └─ config.py # Конфигурация клиента\
│\
├─ docker-compose.yml # Docker-сетап сервера и клиентов\
